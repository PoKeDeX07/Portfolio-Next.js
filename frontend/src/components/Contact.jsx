...
const Contact = () => {
  ...
  const onSubmit = (e) => {
    ...
    setSubmitting(true);
    // Frontend-only: simulate submission and persist to localStorage for now
    setTimeout(() => {
      try {
        const existing = JSON.parse(localStorage.getItem("ak_messages") || "[]");
        existing.push({ ...form, at: new Date().toISOString() });
        localStorage.setItem("ak_messages", JSON.stringify(existing));
      } catch (err) {
        // ignore
      }
      setSubmitting(false);
